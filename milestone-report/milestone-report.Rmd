...
sampled at `r round(sampleSize*100)`% to improve performance. The subset of
data will initially be cleaned by removing all non-English characters and then
combined into a single data set. The new combined sample data set is written to
disk and contains
...
buildCorpus <- function (x) {
    ...
    #corpus <- tm_map(corpus, removeWords, bannedWords) # Eliminate banned words
...
corpusData <- buildCorpus(sampleData)
saveRDS(corpusData, file = "data/final/en_US/en_US.corpus.rds")

# remove variables no longer needed to free up memory
rm(sampleData)
```

The final step in preparing the data will be to create a corpus from the sampled
data set. The `tm` package will be employed to conduct further data cleaning and
...
A custom function named `buildCorpus` will be created to perform the following
cleanup steps:
