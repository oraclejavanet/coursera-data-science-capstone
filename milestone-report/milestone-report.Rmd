...
single data set. The new combined sample data set is written to disk which
contains
...
function named `buildCorpus` will be created and employed to perform the
following transformation steps:

1. Remove URLs, Twitter handles and emails by converting them to spaces
...
1. Trim whitespace
1. Remove profanity
1. Convert the text to plaintext
...
buildCorpus <- function (dataSet) {
    ...
    corpus <- tm_map(corpus, removeWords, c("fuckn",
                                            "fucken",
                                            "fuckkkk",
                                            "fuckd",
                                            "muhfuckas",
...
# convert corpus to a dataframe and write words to disk (text)
corpusText <- data.frame(text = unlist(sapply(corpusData, '[', "content")), stringsAsFactors = FALSE)
...
kable(head(corpusText$text, 10),
      ...
      col.names = NULL,
      align = c("l"),
      caption = "First 10 Lines") %>% kable_styling(position = "left")
